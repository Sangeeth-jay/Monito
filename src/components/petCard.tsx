...
const PetCard: React.FC<PetCardProps> = ({ bread, image, price, gene, age }) => {
  return (
    <div className="md:w-72 w-44 h-80 shadow-lg rounded-xl p-2 flex flex-col items-start justify-between gap-3 hover:shadow-2xl transition-shadow">
      <div className="w-full h-40 bg-slate-300 rounded-2xl">
        <img src={image} alt={`${bread} image`} className="w-full h-full object-cover rounded-2xl" />
      </div>
